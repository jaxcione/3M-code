...
N_y=30 #number of grid points in y direction 
...
"Note:I had AI help me plot this. I beieve ai has superior graphing abilities than me. Nonetheless, I still reviewed the code and was mindful of what it was doing"
#Plotting--------------------------------------------------------------------

#style of all colors etc
plt.rcParams.update({
...
for i, C_plot in enumerate(C_at_time_snaps):
    ...
    cont = ax.contourf(X_full, Y_full * 1000, C_norm, levels=np.linspace(0, 1, 50), cmap=CMAP_HM)
    cbar = fig.colorbar(cont, ax=ax, pad=0.02, fraction=0.03)
    cbar.set_label("C / C₀", labelpad=8, fontsize=10)
    cbar.ax.tick_params(labelsize=8)
    ax.set_title(f"Gas-Phase Concentration  —  t = {snap_labels[i]}",fontweight='semibold', pad=9)
...
if t_sat is not None:
    ...
    ax.text(t_sat + 0.05, 0.88, f"$t_{{sat}}$ = {t_sat:.1f} hr",color="#59a14f", fontsize=9)

style_ax(ax, "Breakthrough Curve — Outlet Concentration vs Time","Time  [hr]", "$C_{out}$ / $C_0$")
...
style_ax(ax, "Spatially Averaged Sorbent Loading vs Time","Time  [hr]", "Loading  [mol m$^{-2}$]")
plt.tight_layout()
plt.show()
# ── 4. Total CO₂ captured ─────────────────────────────────────────────────────
W = 1.0
...
style_ax(ax, "Total CO$_2$ Captured vs Time","Time  [hr]", "CO$_2$ Captured  [mol]")
plt.tight_layout()
plt.show()
# ── 3. Sorbent loading vs time ────────────────────────────────────────────────
time_axis= np.linspace(0, t_f, len(qb_hist))
